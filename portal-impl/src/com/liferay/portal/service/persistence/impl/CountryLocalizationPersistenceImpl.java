...
public class CountryLocalizationPersistenceImpl
	...
	private FinderPath _finderPathFetchByTitle;
	private FinderPath _finderPathCountByTitle;

	/**
	 * Returns the country localization where title = &#63; or throws a <code>NoSuchCountryLocalizationException</code> if it could not be found.
	 *
	 ...
	 */
	@Override
	public CountryLocalization findByTitle(String title)
		throws NoSuchCountryLocalizationException {

		CountryLocalization countryLocalization = fetchByTitle(title);
	...
	/**
	 * Returns the country localization where title = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param title the title
	 * @return the matching country localization, or <code>null</code> if a matching country localization could not be found
	 */
	@Override
	public CountryLocalization fetchByTitle(String title) {
		return fetchByTitle(title, true);
	}

	/**
	 * Returns the country localization where title = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param title the title
	 * @param useFinderCache whether to use the finder cache
	 * @return the matching country localization, or <code>null</code> if a matching country localization could not be found
	 */
	@Override
	public CountryLocalization fetchByTitle(
		String title, boolean useFinderCache) {
		...
		if (useFinderCache) {
			result = FinderCacheUtil.getResult(
				_finderPathFetchByTitle, finderArgs, this);
		}
		...
		if (result instanceof CountryLocalization) {
			...
			if (!Objects.equals(title, countryLocalization.getTitle())) {
				result = null;
			}
		}
		...
		if (result == null) {
			...
			try {
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						FinderCacheUtil.putResult(
							_finderPathFetchByTitle, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"CountryLocalizationPersistenceImpl.fetchByTitle(String, boolean) with parameters (" +
	...
	/**
	 * Removes the country localization where title = &#63; from the database.
	 *
	 * @param title the title
	 * @return the country localization that was removed
	 */
	@Override
	public CountryLocalization removeByTitle(String title)
		throws NoSuchCountryLocalizationException {

		CountryLocalization countryLocalization = findByTitle(title);

		return remove(countryLocalization);
	}

	/**
	 * Returns the number of country localizations where title = &#63;.
	 *
	 * @param title the title
	 * @return the number of matching country localizations
	 */
	...
	private static final String _FINDER_COLUMN_TITLE_TITLE_2 =
		"countryLocalization.title = ?";

	private static final String _FINDER_COLUMN_TITLE_TITLE_3 =
		"(countryLocalization.title IS NULL OR countryLocalization.title = '')";
	...
	public void cacheResult(CountryLocalization countryLocalization) {
		...
		FinderCacheUtil.putResult(
			_finderPathFetchByTitle,
			new Object[] {countryLocalization.getTitle()}, countryLocalization);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		args = new Object[] {countryLocalizationModelImpl.getTitle()};

		FinderCacheUtil.putResult(
			_finderPathCountByTitle, args, Long.valueOf(1));
		FinderCacheUtil.putResult(
			_finderPathFetchByTitle, args, countryLocalizationModelImpl);
	}
	...
	public void afterPropertiesSet() {
		...
		_finderPathFetchByTitle = new FinderPath(
