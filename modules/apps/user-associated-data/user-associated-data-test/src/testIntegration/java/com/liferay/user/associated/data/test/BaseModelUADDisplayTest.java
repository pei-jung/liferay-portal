...
public class BaseModelUADDisplayTest {
	...
	public void setUp() throws Exception {
		...
		_uadDisplay = new TestRoleUADDisplay();
	...
	public void testOrder() throws Exception {
		...
		_assertOrder("name", "asc", Arrays.asList(layout1, layout2, layout3));
		_assertOrder("name", null, Arrays.asList(layout1, layout2, layout3));
		_assertOrder("name", "desc", Arrays.asList(layout3, layout2, layout1));
		_assertOrder("title", "asc", Arrays.asList(layout3, layout1, layout2));
		_assertOrder("title", null, Arrays.asList(layout3, layout1, layout2));
		_assertOrder("title", "desc", Arrays.asList(layout2, layout1, layout3));
		_assertOrder(
			"description", "asc", Arrays.asList(layout2, layout3, layout1));
		_assertOrder(
			"description", null, Arrays.asList(layout2, layout3, layout1));
		_assertOrder(
			"description", "desc", Arrays.asList(layout1, layout3, layout2));

		_uadDisplay.setGetOrderByComparatorBiFunction(
	...
	public void testSearchByGroupId() throws Exception {
		...
		Layout excludedGroup1Layout = _addLayout(
			TestPropsValues.getUserId(), groupId);
		Layout excludedGroup2Layout = _addLayout(
			TestPropsValues.getUserId(), group2.getGroupId());
		Layout group1Layout = _addLayout(userId, groupId);
		Layout group1Layout1 = _addLayout(userId, groupId);
		Layout group2Layout = _addLayout(userId, groupId2);
		Layout group2Layout2 = _addLayout(userId, groupId2);

		List<Layout> layouts = _searchGroupLayouts(groupId);

		Assert.assertFalse(layouts.contains(excludedGroup1Layout));
		Assert.assertFalse(layouts.contains(excludedGroup2Layout));
		Assert.assertTrue(layouts.contains(group1Layout));
		Assert.assertTrue(layouts.contains(group1Layout1));
		Assert.assertFalse(layouts.contains(group2Layout));
		Assert.assertFalse(layouts.contains(group2Layout2));

		Assert.assertEquals(2, _searchCountGroupLayouts(groupId));

		layouts = _searchGroupLayouts(groupId2);

		Assert.assertFalse(layouts.contains(excludedGroup1Layout));
		Assert.assertFalse(layouts.contains(excludedGroup2Layout));
		Assert.assertFalse(layouts.contains(group1Layout));
		Assert.assertFalse(layouts.contains(group1Layout1));
		Assert.assertTrue(layouts.contains(group2Layout));
		Assert.assertTrue(layouts.contains(group2Layout2));

		Assert.assertEquals(2, _searchCountGroupLayouts(groupId2));

		layouts = _searchGroupLayouts(groupId, groupId2);
	...
	private void _assertOrder(
		String orderByCol, String orderByType, List<Layout> expected) {

		List<Layout> actual = _uadDisplay.search(
			_user.getUserId(), null, null, orderByCol, orderByType,
			QueryUtil.ALL_POS, QueryUtil.ALL_POS);

		Assert.assertEquals(actual.toString(), expected.size(), actual.size());
	...
	private TestRoleUADDisplay _uadDisplay;

	@DeleteAfterTestRun
	private User _user;

	private class TestRoleUADDisplay extends BaseModelUADDisplay<Layout> {
		...
		public void setGetOrderByComparatorBiFunction(
			BiFunction<String, String, OrderByComparator<Layout>>
				getOrderByComparatorBiFunction) {

			_getOrderByComparatorBiFunction = getOrderByComparatorBiFunction;
		}
		...
		protected OrderByComparator<Layout> getOrderByComparator(
			...
			if (_getOrderByComparatorBiFunction != null) {
				return _getOrderByComparatorBiFunction.apply(
		...
		private BiFunction<String, String, OrderByComparator<Layout>>
			_getOrderByComparatorBiFunction;
