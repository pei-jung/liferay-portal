...
public class BookmarksEntryUADEntityAggregator extends BaseUADEntityAggregator {
	...
	public List<UADEntity> getUADEntities(long userId) {
		...
		for (BookmarksEntry bookmarksEntry : bookmarksEntries) {
			uadEntities.add(
				_createBookmarksEntryUADEntity(userId, bookmarksEntry));
		}
	...
	public UADEntity getUADEntity(String uadEntityId) throws PortalException {
		...
		return _createBookmarksEntryUADEntity(
			_getUserId(uadEntityId), bookmarksEntry);
	}

	private BookmarksEntryUADEntity _createBookmarksEntryUADEntity(
	...
	private String _getUADEntityId(long userId, BookmarksEntry bookmarksEntry) {
		return Long.toString(bookmarksEntry.getEntryId()) + StringPool.POUND +
