...
<c:choose>
	<c:when test="<%= showList %>">
		<liferay-frontend:management-bar
			...
			searchContainerId="organizationMembers"
		>
			...
			<liferay-frontend:management-bar-action-buttons>
				<liferay-frontend:management-bar-button href='<%= "javascript:" + renderResponse.getNamespace() + "deleteMembers();" %>' icon="trash" id="deleteOrganizations" label="delete" />
			</liferay-frontend:management-bar-action-buttons>
		</liferay-frontend:management-bar>
		...
		<aui:form action="<%= portletURL.toString() %>" cssClass="container-fluid-1280" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "search();" %>'>
			...
			<liferay-ui:search-container
				...
				rowChecker="<%= new OrganizationMemberChecker(renderResponse) %>"
...
<aui:script>
	function <portlet:namespace />deleteMembers() {
